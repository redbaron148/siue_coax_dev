...
(defclass <CoaxState> (ros-message)
  ((header
   ...
   (coaxspeed
    :reader coaxspeed-val
    :initarg :coaxspeed
    :type coax_msgs-msg:<CoaxSpeed>
    :initform (make-instance 'coax_msgs-msg:<CoaxSpeed>))
   (o_attitude
...
(defmethod serialize ((msg <CoaxState>) ostream)
  ...
  (serialize (slot-value msg 'coaxspeed) ostream)
    (map nil #'(lambda (ele) (let ((bits (roslisp-utils:encode-single-float-bits ele)))
    (write-byte (ldb (byte 8 0) bits) ostream)
    (write-byte (ldb (byte 8 8) bits) ostream)
    (write-byte (ldb (byte 8 16) bits) ostream)
...
(let ((bits 0))
    ...
    (setf (ldb (byte 8 24) bits) (read-byte istream))
    (setf (aref vals i) (roslisp-utils:decode-single-float-bits bits)))))
  (deserialize (slot-value msg 'coaxspeed) istream)
...
(defmethod serialization-length ((msg <CoaxState>))
  (+ 0
     ...
     (serialization-length (slot-value msg 'coaxspeed))
...
(defmethod ros-message-to-list ((msg <CoaxState>))
  ...
  (list '<CoaxState>
    ...
    (cons ':coaxspeed (coaxspeed-val msg))
