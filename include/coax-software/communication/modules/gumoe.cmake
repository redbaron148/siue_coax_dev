...
# Search HOME for "gumstix-oe" directory. First found directory is taken.
# Might need to "updatedb"...
# NOTE: Unfortunately FIND_PATH does not allow searching recursively
MESSAGE( STATUS "Searching gumstix-oe directory..." )
EXECUTE_PROCESS( COMMAND find $ENV{HOME} -type d -name "*gumstix-oe" 
		 COMMAND head -n 1
		 COMMAND tr -d '\n'
		 OUTPUT_VARIABLE GUM_ROOTPATH )

IF( NOT GUM_ROOTPATH )
  MESSAGE( FATAL_ERROR "gumstix-oe directory not found. Aborting." )
...
EXECUTE_PROCESS( COMMAND find ${GUM_ROOTPATH} -name "${BIN_PREFIX}-gcc"
		 COMMAND head -n 1
		 COMMAND tr -d '\n'
		 OUTPUT_VARIABLE GUM_GCC )

IF( NOT GUM_GCC )
  MESSAGE( FATAL_ERROR "${BIN_PREFIX}-gcc compiler not found. Aborting." )
ENDIF( NOT GUM_GCC )
...
EXECUTE_PROCESS( COMMAND find ${GUM_ROOTPATH} -name "${BIN_PREFIX}-g++"
		 COMMAND head -n 1
		 COMMAND tr -d '\n'
		 OUTPUT_VARIABLE GUM_GXX )

IF( NOT GUM_GXX )
  MESSAGE( FATAL_ERROR "${BIN_PREFIX}-g++ compiler not found. Aborting." )
ENDIF( NOT GUM_GXX )
