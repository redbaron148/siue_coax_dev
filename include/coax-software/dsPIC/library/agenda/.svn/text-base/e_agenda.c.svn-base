...
#include "configs/coax_ports.h"		// TO DELETE AFTER T2 period test !!!!!!!!
...
{
	T6CON = 0;							// reset Timer6 CONtrol register
	T6CONbits.TCKPS = 0;				// precsaler = 1
	TMR6 = 0;							// clear timer 6
	PR6 = (int)(_100MICROSEC);			// interrupt every 0.1 ms with 64 prescaler
	IFS2bits.T6IF = 0;					// clear interrupt flag
	IEC2bits.T6IE = 1;					// set interrupt enable bit
...
void __attribute__((interrupt, auto_psv)) _T6Interrupt(void)
{
	...
	// -------_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)_T2Interrupt(void)
#endif
