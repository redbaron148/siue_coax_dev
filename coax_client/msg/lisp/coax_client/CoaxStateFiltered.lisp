...
(defclass <CoaxStateFiltered> (ros-message)
  ((header
   ...
   :initform (make-array 3 :element-type 'float :initial-element 0.0))
   (global_accel
    :reader global_accel-val
    :initarg :global_accel
    :type (vector float)
   :initform (make-array 3 :element-type 'float :initial-element 0.0)))
)
(defmethod serialize ((msg <CoaxStateFiltered>) ostream)
  ...
  (serialize (slot-value msg 'header) ostream)
    ...
    (write-byte (ldb (byte 8 8) bits) ostream)
    (write-byte (ldb (byte 8 16) bits) ostream)
    (write-byte (ldb (byte 8 24) bits) ostream)))(slot-value msg 'global_accel))
)
...
(let ((bits 0))
    ...
    (setf (ldb (byte 8 24) bits) (read-byte istream))
    (setf (aref vals i) (roslisp-utils:decode-single-float-bits bits)))))
  (setf (slot-value msg 'global_accel) (make-array 3))
  (let ((vals (slot-value msg 'global_accel)))
    (dotimes (i 3)
(let ((bits 0))
    (setf (ldb (byte 8 0) bits) (read-byte istream))
    (setf (ldb (byte 8 8) bits) (read-byte istream))
...
(defmethod md5sum ((type (eql '<CoaxStateFiltered>)))
  ...
  "5dca3b22a67c7984059c8139d31fd3c0")
...
(defmethod serialization-length ((msg <CoaxStateFiltered>))
  (+ 0
     ...
     0 (reduce #'+ (slot-value msg 'global_accel) :key #'(lambda (ele) (declare (ignorable ele)) (+ 4)))
))
(defmethod ros-message-to-list ((msg <CoaxStateFiltered>))
  ...
  (list '<CoaxStateFiltered>
    ...
    (cons ':global_accel (global_accel-val msg))
))
